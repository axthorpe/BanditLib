...
class simulateOnlineData(object):
	...
	def CoTheta(self):
		for ui in self.users:
			...
			for uj in self.users:
				print('UJTheta: ' + str(np.asarray(uj.theta).shape))
				print('uiCotheta: ' + str(ui.CoTheta.shape))
				print('W: ' + str((self.W[uj.id][ui.id]).shape))
				print((self.W[uj.id][ui.id] * np.asarray(uj.theta)).shape)
	...
	def regulateArticlePool(self, currentUser):
		# Randomly generate articles evenly from different article types
	...
	def transitionProbability(self, position, Article, user, noise):
		test = np.dot(user.beta, Article.returnFeatureVector) # Trying out position independent beta
		return 1/(1+math.exp(-1.0*(test + noise)))
	...
	def getClick(self, user, Article, noise):
		...
		clickProb = relevance	
		# if relevance <= 0.5:
		# 	clickProb = 0.15#user.profile[0]
		# else:
		# 	clickProb = 0.9#user.profile[1]
	...
	def getReward2(self, pickedArticles, clicks, u, noise):
		rewards = []
		for i,article in enumerate(pickedArticles):
			if random.random() > self.transitionProbability(i, article, u, noise):
				break
	...
	def GetOptimalReward3(self, clicks):
		rewards = []
		for article in self.articlePool:
			if clicks[article.id]:
				rewards.append(1)
	...
	def runAlgorithms(self, algorithms):
			...
			for iter_ in range(self.testing_iterations):
				...
				for u in self.users:
					...
					self.regulateArticlePool(u) # select random articles
	...
	def runAlgorithms2(self, algorithms):
		...
		for iter_ in range(self.testing_iterations):
			...
			for u in self.users:
				...
				#get optimal reward for user x at time t
				OptimalRewards = self.GetOptimalReward2(clicks) 
				# OptimalReward += noise
				for alg_name, alg in algorithms.items():
					pickedArticles = alg.decide(self.articlePool, u.id)
					rewards = self.getReward2(pickedArticles, clicks, u, noise)
					# reward += noise
					#BUG Divya: no update for previous 0 click arms!
					if (self.testing_method=="online"): # for batch test, do not update while testing
						alg.updateParameters(pickedArticles, rewards, u.id)
						if alg_name =='CLUB':
							n_components= alg.updateGraphClusters(u.id,'False')
...
if __name__ == '__main__':
	...
	userFilename = os.path.join(sim_files_folder, "users_" + 'featureUniform'+str(n_users)+"+dim--"+str(dimension) +".json")
	...
	#for i in range(len(users)):
	i= 0
	#print users[-1].returnProb

	atypes = ['smallTheta_smallBeta', 'smallTheta_largeBeta', 'largeTheta_smallBeta', 'largeTheta_largeBeta']
	for i in range(len(users)):
		articlesFilename = os.path.join(sim_files_folder,  'Neg' + 'userindex' + str(len(users)) +'_' + str(i) + "articles_"  + 'featureUniform'+str(n_articles)+"+dim"+str(dimension) + 'TwoFeature'+".json")
		...
		print("outermost loop iter " + str(i))
		...
		#articles = articles_large_large + articles_large_small + articles_small_small
		#atypes = ['smallTheta_smallBeta', 'smallTheta_largeBeta', 'largeTheta_smallBeta']
		
		#articles = articles_large_large + articles_large_small
		#articles = articles_small_large + articles_small_small 
		#articles = articles_small_large + articles_large_small
		#articles = articles_large_large
		#articles = articles_large_small
		articlesList = []
		...
		users[i].setArticleList(articlesList)


	flatten = lambda l: [item for sublist in l for item in sublist]
	articles = flatten(articlesList)
	...
	if algName == 'CascadeUCB':
		...
		topK = True
	if algName == 'CascadeUCB_top2':
		# algorithms['CascadeUCB'] = CascadeUCBAlgorithm(itemNum = n_articles, n = n_users, K=K)
		algorithms['Top2_permutation'] = PermutationLinUCBAlgorithm(dimension = context_dimension, alpha = alpha, lambda_ = lambda_, n = n_users, K=K)
		algorithms['LinUCB'] = Top2LinUCBAlgorithm(dimension = context_dimension, alpha = alpha, lambda_ = lambda_, n = n_users, K=K)
		topK = True
	if algName == 'LinUCB':
